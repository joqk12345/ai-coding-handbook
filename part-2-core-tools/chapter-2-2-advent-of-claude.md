...
### Claude Code 2.1.3 说明：Slash Commands 与 Skills 的合并

> **在 Claude Code 2.1.3 版本中，把“slash commands（斜杠命令）”和“skills（技能）”合并成了一个统一机制，从而简化了用户的理解模型，但功能和行为本身没有变化。**

这句话可以拆成 4 个关键点：

1. **Slash commands 是调用入口**：即以 `/` 开头的命令，例如 `/edit`、`/test`、`/fix`。  
2. **Skills 是能力单元**：例如代码重构、测试生成、调试分析等可复用能力模块。  
3. **2.1.3 做的是概念合并**：以前“命令层”和“技能层”分开描述，现在统一成同一机制。  
4. **行为不变**：旧命令仍可用、输出逻辑保持一致，本质属于“重构而非功能变化”。

可以用一个类比快速理解：

- 以前：`按钮（slash） → 调度层 → 技能模块`
- 现在：`按钮（slash） = 技能调用入口`

这就是官方所说的 **simplifying the mental model**：用户不必再区分两套概念，只要记住“所有能力都可通过统一机制调用”。
