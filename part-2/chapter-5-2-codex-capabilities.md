...
它首先解决的其实不是“组织结构”问题，而是**上下文预算**问题。LLM 的能力这两年提升很快，但可稳定利用的上下文窗口并没有同步无限扩张。理论上有些模型已经接近百万 token，但真正稳定、高质量的结果，往往仍然出现在远低于上限的上下文占用下。于是，凡是高检索量、高噪声输出、高中间状态消耗的任务，都值得优先交给 subagent 去吃掉。

换句话说，subagent 的核心价值不是“分身术”，而是**给主 agent 换来一块干净的新上下文**。父代理只保留目标、约束和最终决策，把探索过程、测试日志、局部调试链路这些 token 黑洞外包出去。
...
Claude Code 的 `Explore` 子代理很好地说明了 subagent 的默认用途。面对一个陌生代码库时，主 agent 往往不会立刻改代码，而是先派一个探索型子代理去完成 repo reconnaissance：查目录结构、找模板、找样式、找生成 diff 的后端代码、找相关关键词，然后把结果整理成一份结构化摘要返回。

这种模式很像一次高级工具调用。主 agent 发出任务，子 agent 用一块全新的上下文窗口自行搜索、阅读、筛选，再把“关键文件、关键函数、关键线索”压缩成短报告回传。父代理真正消耗的，不是整段探索过程，而是探索结论。

这也是为什么 `Explore` 类子代理常常比“并行写代码”更稳。它先把昂贵的上下文探索成本隔离掉，再让主 agent 在更清晰的局面下做编辑、重构或修复。

### 常见专职子代理

- **explorer**：负责代码库探索、差异定位、文档检索、依赖关系梳理。
- **test runner**：负责跑测试和消化冗长日志，只把失败用例、报错摘要和可疑位置返回给主 agent。
- **reviewer**：负责做缺陷扫描、设计审查、边界条件核对。
- **debugger**：负责集中消耗 token 做复现路径、根因假设和实验验证。

### 价值

- **保护顶层上下文**：把探索、检索、测试、审查等子任务放进独立上下文，减少主线程被无关细节污染。
- **吸收高噪声输出**：像测试日志、搜索结果、长 diff、排查过程这类内容，可以先由子代理消化后再回传摘要。
...
- 优先把 subagent 用在“读多写少”或“输出极长”的任务上，例如代码库探索、差异比较、日志分析、文档汇总和测试排查。
- custom agent 的说明要聚焦“职责边界”和“输出格式”，而不是写成长篇泛化人格描述。
- 对写操作保持收敛：让 explorer、reviewer 一类 agent 尽量只读，把最终修改集中回主 agent 或单一执行 agent。
- explorer、test runner、debugger 这类子代理的回传格式最好固定成“结论摘要 + 关键证据 + 下一步建议”，避免把压缩不充分的原始噪声重新塞回主上下文。
...
- 不要为了形式把任务拆成几十个专职角色。subagent 的主要收益来自上下文管理和高噪声任务隔离，而不是把任何动作都包装成一个新 agent。
